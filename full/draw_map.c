...
/*   Updated: 2023/03/04 18:25:39 by abouabra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void shoot_ray(t_vars *vars)
{
	...
	while(r < FOV)
	{
		...
		while(j < PIXEL_WIDTH)
		{
			...
			draw_line(vars, i+j, 0, i+j, celing, get_celling_color(vars));
			
			draw_line(vars, i+j, floor, i+j, vars->number_of_lines * 64, get_floor_color(vars));
			
			draw_line(vars, i+j,  LineO, i+j,vars->store[r] + LineO, vars->wall_colors[r]);
			j++;
		}
		i+= PIXEL_WIDTH;
		r++;
